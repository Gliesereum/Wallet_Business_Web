...
import {Button, Input, Checkbox, Form as AntdForm} from 'antd';
...
class Index extends Component<P, S> {
  ...
  renderServiceItem = (service, packageServicesList, setFieldValue) => {
    ...
    return (
      <div className="coupler-packages-service-prices-item" key={service.id}>
        ...
        >
          <p>{service.name}</p>
          <p>{service.price.toFixed(2)} грн.</p>
        </Checkbox>
      </div>
    )
  };
  ...
  renderField = ({type, error, touched, value, label, name, setFieldValue}) => {
    if (type === 'text') {
      return (
          ...
          render={({errors, touched, values, setFieldValue}) => (
            <Form>
              {renderFields.map(fieldProps => (
                <Field key={fieldProps.key} name={fieldProps.key} render={({field}) => (
                  <RenderField {...{
                    ...fieldProps, ...field,
